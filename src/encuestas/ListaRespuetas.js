...
export const ListaRespuestas =(props)=>{
  ...
  const[listaRespuestas,setListaRespuestas]=useState([]);
    ...
    useEffect (() => {
        ...
        const loadRespuesta=()=>{
            ...
            //};
            Axios.get("http://localhost:8000/api/respuestas/" )
            .then((response)=>{
            console.log(response.data); 
            setListaRespuestas(response.data.data);   
            //}).catch(error => {
        ...
        const eliminarRespuesta = (id) =>{
           // const headers = {
             //   "Authorization" :"Bearer " + token
           // };
            let confirmation =window.confirm('¿Está seguro que desea eliminar el Respuesta?');
            if(!confirmation){
            return;
            }

        Axios.delete('http://localhost:8000/api/respuestas'+id )
        .then(response=>{
            ...
            console.log(response.data);
            alert('Hubo un error al borrar una respuestas');
            }
       // }).catch(error => {
           // if(error.response.status === 401){
            //    history.push('/login');
